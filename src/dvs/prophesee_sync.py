...
class PropheseeCameraSync(AbstractProcess):
    """
...
@implements(proc=PropheseeCameraSync, protocol=LoihiProtocol)
@requires(CPU)
...
class PyPropheseeCameraEventsIteratorModel(PyLoihiProcessModel):
    ...
    t_out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, int)
...
class PyPropheseeCameraRawReaderModelSync(PyLoihiProcessModel):
    s_out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, np.int32)
    t_out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, int)
    ...
    def run_spk(self):
        ...
        if len(events) == 0:
            ...
            l_event = events[-1]
            self.last_time_stamp = l_event[3]*1000
        # Transform to frame
        if len(events) > 0:
        ...
        self.s_out.send(frames)
        self.t_last_iteration = time.time_ns()
        self.t_out.send(np.array([self.last_time_stamp]))
